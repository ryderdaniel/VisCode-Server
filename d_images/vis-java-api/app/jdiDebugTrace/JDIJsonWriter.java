...
public class JDIJsonWriter {
	...
	public void createClassInfo(Class c) throws JSONException,SecurityException
	{
		...
		JSONArray classMember = new JSONArray();
		JSONArray classFunction = new JSONArray();
		Field[] classFields = c.getDeclaredFields();
		for (Field f : classFields)
		{
			JSONObject memberInfo = new JSONObject();
			memberInfo.put("class", f.getDeclaringClass().getName());
			memberInfo.put("name",f.getName());
			memberInfo.put("type",f.getGenericType().getTypeName());
			memberInfo.put("access_right",getMethodModifier(f.getModifiers()));
			classMember.put(memberInfo);
		}
		classInfo.put("Member",classMember);
	...
	public void addFunction(String methodName,String accessible,Type returnType) throws JSONException
	{
		...
		currFunc.put("func_class",JSONObject.NULL);
	...
	public void setClassMethod(String className) throws JSONException
	{
		if (currFunc != null)
			currFunc.put("func_class",className);
	}
	public void createHeapReference(HashMap<Long, List<Object>> extrefList) throws JSONException
	{
		for (Map.Entry<Long,  List<Object>> ref : extrefList.entrySet()) 
		{
			createHeapReference(ref.getKey(),ref.getValue());
		}
	}
	private void createHeapReference(Long refId, List<Object> valueList) throws JSONException
	{
