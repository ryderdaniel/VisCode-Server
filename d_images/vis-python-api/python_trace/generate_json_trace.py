...
parser.add_option('-t', '--compact', default=False, action='store_true',
        help='output compact trace.')
parser.add_option('-j', '--json_format_input', default=False, action='store',
        help='JSON list of strings for code and inputs.', dest='json_format_input')
parser.add_option("-f", dest="filename", default=None,
                  help="Load fileName")
parser.add_option("-o", dest="ofilename", default=None,
                  help="outputfilename")
...
if options.usercode:
  ...
  if not options.filename:
    import json
    jsonInput = sys.stdin.readline();
    codeJson = json.loads(jsonInput)
    code = codeJson["code"]
    if "stdin" in codeJson.keys():
        stdin = codeJson["stdin"]
  else:
  ...
  if options.js_varname:
    ...
    sys.stderr = open("compile_exception.log","w")
    lines = pg_logger.exec_script_str_local(code, stdin, options.cumulative, options.heapPrimitives, json_finalizer)
    import JsonStackTrace as JST
    JST.read_json(lines,options.ofilename)
